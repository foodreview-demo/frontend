...
import { Search, Star, MapPin, Sparkles, Loader2 } from "lucide-react"
...
const categories = ["전체", "한식", "일식", "중식", "양식", "카페", "베이커리", "분식"]
...
export default function SearchPage() {
  ...
  const fetchRestaurants = useCallback(async () => {
    ...
    try {
      let result
      if (searchQuery.trim()) {
        result = await api.searchRestaurants(
          searchQuery,
          undefined,
          selectedCategory !== "전체" ? selectedCategory : undefined
        )
      } else {
        result = await api.getRestaurants(
          undefined,
          selectedCategory !== "전체" ? selectedCategory : undefined
        )
      }
